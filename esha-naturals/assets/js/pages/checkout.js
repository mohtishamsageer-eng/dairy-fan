...
(function (E) {
  ...
  const CITIES = [
    ...
    'Gujranwala', 'Sialkot', 'Sargodha', 'Bahawalpur', 'Sukkur', 'Larkana', 'Abbottabad', 'Mardan', 'Sahiwal',
    'Gujrat', 'Sheikhupura', 'Rahim Yar Khan', 'Jhang', 'Dera Ghazi Khan', 'Dera Ismail Khan', 'Kasur', 'Okara',
    'Wah Cantt', 'Mingora', 'Nawabshah', 'Chiniot', 'Kamoke', 'Hafizabad', 'Sadiqabad', 'Burewala', 'Khanewal',
    'Muzaffargarh', 'Mandi Bahauddin', 'Jhelum', 'Attock', 'Chakwal', 'Vehari', 'Mianwali', 'Khairpur', 'Kohat',
    'Swabi', 'Nowshera', 'Charsadda', 'Mansehra', 'Haripur', 'Mirpur (AJK)', 'Muzaffarabad', 'Gilgit', 'Skardu',
    'Gwadar', 'Turbat', 'Taxila', 'Murree', 'Toba Tek Singh', 'Pakpattan', 'Lodhran', 'Narowal', 'Mirpur Khas', 'Thatta'
  ];

  // Prefill from the last order placed on this device (convenience for repeat customers)
  const last = (() => {
    const list = U.store.get('esha_orders_v1', []);
    return Array.isArray(list) && list[0] && list[0].customer ? list[0].customer : {};
  })();
    ...
    root.innerHTML = `<div class="checkout__grid">
      <form class="checkout-form" novalidate data-checkout-form>
        ...
        <section class="form-card" aria-labelledby="h-contact">
          ...
          <div class="field-row">
            ${field('phone', 'Mobile number', `<input class="input" id="f-phone" name="phone" type="tel" inputmode="tel" autocomplete="tel" maxlength="16" required placeholder="03XX XXXXXXX" value="${esc(last.phone || '')}" ${describedBy('phone', true)}>`, { required: true, hint: 'We will call or WhatsApp you on this number to confirm your order.', error: 'Please enter a valid mobile number, e.g. 0300 1234567.' })}
        ...
        <section class="form-card" aria-labelledby="h-payment">
          ...
          <p class="confirm-note">${icon('phone')}<span>After you place your order, our team will call or WhatsApp you to confirm it. Delivery takes <strong>${esc(d.timeText || '')}</strong>.</span></p>
        </section>

        <div class="hp" aria-hidden="true"><label>Company <input type="text" name="company" tabindex="-1" autocomplete="off"></label></div>

        <div class="place-order">
          <button type="submit" class="btn btn--gold btn--lg btn--block" data-place-order></button>
  ...
  function bindForm() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      // Without a Google Sheet, WhatsApp is how the order reaches the store:
      // open it straight away (must happen inside the click, before any await).
      const waUrl = E.orders.whatsappUrl(order);
      if (waUrl && !cfg.orderEndpoint) {
  ...
  E.cart.onChange(() => {
    // Keep the page as-is while an order is being submitted (the cart is cleared just before redirecting)
    const btn = $('[data-place-order]');
    if (btn && btn.classList.contains('is-loading')) return;
    render();
  });

  if (!cfg.whatsappNumber && !cfg.orderEndpoint && window.console) {
    console.warn('[Esha Naturals] No whatsappNumber or orderEndpoint set in assets/js/config.js, so orders cannot reach the store yet.');
  }
