...
(function (E) {
  ...
  function renderPills() {
    ...
    pills.innerHTML = all
      .map(
        (c) => `<a class="pill" href="${urlFor({ category: c.id, sort: state.sort })}" data-category="${c.id}"${
  ...
  function render() {
    ...
    if (cat) {
      ...
      subEl.textContent = 'Pure hair care and cooking oils made with natural ingredients. 100% original, with Cash on Delivery all over Pakistan.';
      crumb.hidden = true;
      intro.hidden = true;
